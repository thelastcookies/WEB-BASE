...
组态的图元与画面文件采取与项目解耦的方式独立部署。

`.env` 配置项 `APP_DIAGRAM_RESOURCE_URL` 指向图元与画面文件的部署根目录。

在该根目录下，应当存在两个目录，`diagrams` 与 `symbols`。 其中，`symbols` 用来存放图元，`diagrams` 用来存放画面文件。

项目中配置组态画面文件路径，都只应配置相对于 `部署根目录/diagrams` 的相对路径。下文中提到的`画面路径`均指该路径。

举例如下：

```
// 部署根目录
/root/web/apache-tomcat/webapps/resources

// 绝对路径
/root/web/apache-tomcat/webapps/resources/diagrams/unit1/sample.cfd
...
项目提供了四种向页面内接入组态画面的方式：

#### 菜单 Action

配置菜单时，`类型`勾选`组态`，`组态路径`中配置对应的画面路径。

如果是静态菜单，则将 `type` 配置为 `MenuTypeEnum.DIAGRAM`，`resource` 配置对应的画面路径。

此种接入方式适合页面为全组态画面，且画面路径固定的情况，将作为固定的菜单存在于项目菜单配置中。

#### 路由 meta

配置菜单时，在`Meta 配置`中新增 `href: 画面路径` 键值对。

如果是静态菜单，则需要增加 `meta` 字段，同样以键值对形式配置

```
meta: {
  href: 'unit1/sample.cfd',
}
```

此种接入方式适合页面中只有部分内容为组态，其他部分均为常规页面，且画面路径固定的情况，将作为固定的菜单存在于项目菜单配置中。

#### 组件 props

此种接入方式更为灵活，可以在任意页面中使用 `Diagram` 组件，将画面路径通过 props 传入组件。

#### url 跳转（编程式导航/组态画面内跳转）

- 编程式导航

项目中包含一个 `name` 为 `DIAGRAM` 的隐藏路由，可以通过调用 `routeTo` 方法，并提供查询参数 `href: 画面路径` 的方式访问。
...
在包含`链接`配置项的图元中配置画面路径，例如`按钮`等。

在组态中点击这些图元会调用 `routeTo` 方法。

该接入方式不需要有实际的页面组件作为支撑，适用于大批量、读配置的组态接入方式。

### 图元设计、配置规约

图元图片严格按照组态编辑器中的图元颜色和尺寸绘制，外观上应无明显差异。

图元图片的导出格式为 `.svg`，图元名称须与编辑器图元配置文件中的名称保持一致。 名称中使用到`（）`的均采用全角字符。

图元存在多种变换状态的，图片命名格式为：`{图元名称}-variant-{编号}`。

编号选择遵循以下规则：

如果图元为单测点 01 值的，编号与测点的 01 值大致对应。例：
...
如果图元需绑定多个 01 值测点，编号由多个测点 01 值组成的二进制数的十进制值决定。例：
...
如果图元有朝向改变需求，则需要绘制所有方向下的图元图片。数字编号遵循顺时针方向依次递增。例：
